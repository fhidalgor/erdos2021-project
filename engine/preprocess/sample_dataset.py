...
class SampleDataset(Preprocess):
    ...
    def get_pairs(self) -> Tuple[list, list]:    
        ...
        """
        df = pd.read_csv(os.path.join(self.input_path, "counts.csv"), converters={'long_forms': eval, 'unique_key': eval})
        df = df.sort_values(by='counts')
        df = df.explode('unique_key')
        df.dropna(inplace=True)
        df.drop_duplicates(subset='unique_key',inplace=True)
        temp = df.groupby(by=['long_form'])['unique_key'].apply(list)
        long_forms: list = list(temp.index)
        keys: list = list(temp)
        return long_forms, keys
                
    def sample_data(self, input: List[tuple]) -> None:
        """
        ...
        print (len(input))
        for long_form, keys_subset in input:
            for i, key in enumerate(keys_subset):
                ...
                if i > self.max_long_form_occurrences:
                    ...
                    row = self.replaced_dfs[key.split('_')[0]].loc[self.replaced_dfs[key.split('_')[0]]['unique_key']==key]
                    index_long_form = list(row['long_forms'])[0].index(long_form)
                    span: Tuple[int, int] = list(row['span_short_form'])[0][index_long_form]
                    df_results = df_results.append({
                        'long_forms': long_form, 'span_short_form': span,
                        'replaced_abstract': list(row['replaced_abstract'])[0], 'unique_key': key
                    },ignore_index=True)
                                
        # append to csv
        df_results.to_csv(os.path.join(self.output_path, "sampled_dataset.csv"), index = False, header = False, mode = 'a')
    ...
    def single_run(self, filename: str) -> None:
        ...
        if filename != 'counts.csv':
            df_replaced = pd.read_csv(os.path.join(self.input_path, filename), converters={'long_forms': eval, 'span_short_form': eval})
    ...
    def batch_run_sample(self) -> None:
        ...
        arguments:List[Tuple] = [(long_form, keys_subset) for long_form, keys_subset in zip(long_forms, keys)]
